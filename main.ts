...
namespace Tinybit_ACE {
    ...
    //% blockId=Tinybit_CarCtrl block="Car Control|%index"
    //% weight=5
    //% blockGap=10
    //% color="#585CA9"
...
export function moveRobotBySteps(direction: Direction, steps: number): void {
    for (let i = 0; i < steps; i++) {
        if (direction === Direction.Forward) {
            setPwmMotor(1, 80, 80); // Move forward
        } else if (direction === Direction.Backward) {
            setPwmMotor(2, 80,80); // Move backward
        }
        basic.pause(750); // Pause for visibility of each step
        // Stop motors after each step
        setPwmMotor(0, 0, 0); // Stop all motors
        basic.pause(500); // Short pause before next step
    }
}
